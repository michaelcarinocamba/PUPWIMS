...
$query = mysqli_query($conn, "SELECT * FROM adviser_info");
$row = mysqli_fetch_array($query);
$user_to = $row['facultynum'];
...
if(isset($_POST["post"])){
    ...
    if ($content != "" && $fileName == "") {
        ...
        $query = mysqli_query($conn, "INSERT INTO post VALUES('', '$content' , '$student' , '$name', '$code', '$date_added', 'no','', '', '')");
        $getadviser = mysqli_query($conn, "SELECT * FROM create_class WHERE class_code = '$code' AND student_list LIKE '%$student%'");
        $getadviser_row = mysqli_fetch_array($getadviser);
        $faculty_num = $getadviser_row['facultynum'];

        $getadvisername = mysqli_query($conn, "SELECT * FROM adviser_info WHERE facultynum = '$faculty_num'");
        $getadvisername_row = mysqli_fetch_array($getadvisername);
        $adviser_name = $getadvisername_row['name'];
    
        $notif_desc = "has posted something in discussion.";
        $date_time_now = date("Y-m-d H:i:s");
        

        if($name !== $adviser_name){
            $notif_link = "Adviser_DashHome.php?classCode=$code&#viewpost";
            $getnotif = mysqli_query($conn, "INSERT INTO notification VALUES('', '$notif_desc', '$notif_link', '$name', '$adviser_name', '$code', '$date_time_now','no')");
        }
        header("Location: Student_DashHome.php?classCode=$code&uploadsuccess");
    }
    else if($fileName !=  ""){
    if(in_array($fileActualExt, $allowed)) {
        if($fileError === 0) {
            if($fileSize < 20000000) {
                ...
                $sql = "INSERT INTO post VALUES ('', '$content', '$student', '$name', '$code', '$date_added', 'no' ,'$fileName', '$fileNameNew', '$fileDestination')";
                ...
                header("Location: Student_DashHome.php?classCode=$code&uploadnotsuccess5");
            }
        }else {
            echo "There was an error uploading your file.";
            header("Location: Student_DashHome.php?classCode=$code&uploadnotsuccess6");
        }
    } else{
        echo "You cannot upload files of this type!";
        header("Location: Student_DashHome.php?classCode=$code&uploadnotsuccess7");
    }
    ...
    else
    {
        ...
        header("Location: Student_DashHome.php?classCode=$code&uploadnotsuccess8");
    }
}
...
if(isset($_REQUEST['delete']))
{
    ...
    header("Location: Student_DashHome.php?classCode=$code&deletesuccess");
    }
    else
    {
        $query = mysqli_query($conn, "DELETE FROM post WHERE post_id='$id'");
...
if(isset($_REQUEST['edit']))
{
    ...
    if ($content != "" && $fileName == "") {
        ...
        header("Location: Student_DashHome.php?classCode=$code&editsuccess");
    }
    else if($fileName !=  ""){
    if(in_array($fileActualExt, $allowed)) {
        if($fileError === 0) {
            if($fileSize < 20000000) {
                ...
                echo "Your file is too big!";
                header("Location: Student_DashHome.php?classCode=$code&editnotsuccess1");
            }
        }else {
            echo "There was an error uploading your file.";
            header("Location: Student_DashHome.php?classCode=$code&editnotsuccess2");
        }
    } else{
        echo "You cannot upload files of this type!";
        header("Location: Student_DashHome.php?classCode=$code&editnotsuccess3");
    }
    ...
    {
        echo "Try to input  text or file first";
        header("Location: Student_DashHome.php?classCode=$code&editnotsuccess4");
    }
}
...
if (isset($_GET['post_id'])) {
    ...
    if(isset($_POST['postComment' . $post_id]))
    {
        ...
        $insert_comment = mysqli_query($conn, "INSERT INTO comment VALUES('', '$post_id', '$post_body','$student','$post_name', '$name', '$code', '$date_time_now', 'no')");
            ...
            //Timeframe
                ...
                if(isset($_REQUEST['comment'])){
                    ?>
                        ...
                        <?php 
                        if($student == $posted_by){?>
                        <a class = 'deletebtn' href='Student_Post.php?classCode=<?php echo $code; ?>&editcomment=<?php echo $id ?>&post_id=<?php echo $post_id ?>'>
                    ...
                    ?>
                        ...
                        if(isset($_REQUEST['editcomment']) && isset($_REQUEST['post_id']))
                        {
                            ...
                            if(isset($_POST['update' . $editid]))
                            {
                                ...
                                header("Location: Student_Post.php?comment&post_id=$post_id&classCode=$code");
                            }
                            ...
                            if($editid){
                                ?>
                                    ...
                                    <a class = 'deletebtn' href='Student_Post.php?classCode=<?php echo $code; ?>&deletecomment=<?php echo $id ?>&post_id=<?php echo $post_id ?>'>
                                        <i class = 'fa fa-trash'></i>
                                    </a>
                                    
                                    <a class = 'deletebtn' href='Student_Post.php?classCode=<?php echo $code; ?>&editcomment=<?php echo $id ?>&post_id=<?php echo $post_id ?>' data-bs-toggle="modal" data-bs-target="#exampleModal">
                            ...
                            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <form action="Student_Post.php?post_id=<?php echo $post_id; ?>&editcomment=<?php echo $editid ?>" name="editComment<?php echo $editid ?>" method="POST" autocomplete="off">
...
if(isset($_REQUEST['comment'])){
   ?>
                        <form action="Student_Post.php?comment&post_id=<?php echo $post_id; ?>&classCode=<?php echo $code ?>" id="comment_form" name="postComment<?php echo $post_id; ?>" method="POST" autocomplete="off">
