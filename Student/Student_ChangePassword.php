<?php
    ...
    if(isset($_POST["password"]))
    {
        ...
        $conf_password = $_POST["conf_password"];
            ...
            {
                header("Location: Student_ChangePassword.php?error2=Password must contain atleast 8 characters.");
            }
            else if(!preg_match('/[A-Z]/', $password))
            {
                header("Location: Student_ChangePassword.php?error3=Password must contain atleast 1 Capital Letter.");
            }
            else if(!preg_match('/[0-9]/', $password)) 
            {
                header("Location: Student_ChangePassword.php?error4=Password must contain atleast 1 Number.");
            }
            else if($password === $conf_password)
            {
                
                $query = mysqli_query($conn, "UPDATE student_info SET password = '$password', change_pass = 'yes' WHERE studentnum = '$studentnum'");
            ...
            {
                header("Location: Student_ChangePassword.php?error1=Password and Confirm Password doesn't match");
                exit();
            }
